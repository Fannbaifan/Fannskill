#!/usr/bin/env python3
"""
抖音创作者中心 - 观众画像 + 导出数据抓取 v6
点击投稿列表后强制等待4秒，不再做特征判断
"""
...
def main():
    ...
    with sync_playwright() as p:
        ...
        video_ids = page.evaluate('''() => {
            ...
            if (ids.length === 0) {
                document.querySelectorAll('[data-item-id]').forEach(el => {
                    ...
                    if (id && !ids.find(x => x.id === id)) {
                        ids.push({id: id, title: el.textContent.trim().slice(0,80)});
                    }
                });
            }
            return ids;
        ...
        print("\n抓取观众画像...")
        results = []
        for i, v in enumerate(video_ids[:15]):
